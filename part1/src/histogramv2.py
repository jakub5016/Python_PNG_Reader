...
def histogram(palette, hist_chunk):
    ...
    for i, (component, color) in enumerate(zip(['Red', 'Green', 'Blue'], ['red', 'green', 'blue'])):
        axs[(i+1)//2, (i+1)%2].hist([color[i] for color in colors_rgb], bins=20, color=color, alpha=0.7)
