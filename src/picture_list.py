from .hex_functions import delete_spaces_from_hex, add_spaces_to_hex
import struct
...
class PictueList(list):
    def __init__(self, hex_no_space):
        ...
        while cursor < len(hex_no_space):
            ...
            if (chunk_type == "IEND") and (
                hex_no_space[cursor : cursor + CRC_SIZE] == "AE426082"
            ):
    ...
    def delete_chunk(self, chunk_name, verbose=True):
        if (
            ...
            or chunk_name == "IHDR"
            or chunk_name == "IEND"
            or (chunk_name == "PLTE" and self.color_type == 3)
        ):
            if verbose == True:
